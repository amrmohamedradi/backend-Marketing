 ...
 */
export function normalizeServices(services: any[]): any[] {
  ...
  return services.map(service => {
    ...
    if (Array.isArray(service.items)) {
      normalizedService.items = service.items.map((item: any) => {
    ...
    if (Array.isArray(service.subServices)) {
      normalizedService.subServices = service.subServices.map((subService: any) => {
